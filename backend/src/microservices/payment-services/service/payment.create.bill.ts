...
class PaymentService {
  ...
  /**
   * Create bill payment allocations with proper tracking
   * 
   * @description Creates PaymentAllocation records and updates bill summaries
  ...
  public async createBillPayment({
    ...
    const billIds = invoicePayments?.map((p) => p.invoiceId);
    ...
    // Batch update bills with new allocations using bulkWrite
...
export const createBillPayment = paymentService.createBillPayment.bind(paymentService);
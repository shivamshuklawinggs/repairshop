...
const journalEntrySchema = yup.object().shape({
  ...
  totalDebit: yup.number().default(0),
  totalCredit: yup.number().default(0),
  ...
  .transform((value) => {
    const totalDebit = value?.entries?.reduce((sum:number, entry:{debit:number}) => sum + (entry.debit || 0), 0) || 0
  const totalCredit = value?.entries?.reduce((sum:number, entry:{credit:number}) => sum + (entry.credit || 0), 0) || 0

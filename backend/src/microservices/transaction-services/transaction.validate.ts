...
const transactionSchema = yup.object().shape({
    customerId: yup.string().label("Customer").test('is-valid-customer', 'Invalid customer', (value) => {
      return mongoose.isValidObjectId(value);
    }).required('Customer is required'),
    type: yup.string().oneOf(["vendor", "customer"]).label("Type").required('Subject is required'),
});

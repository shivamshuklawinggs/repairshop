...
interface ReqQuery {
  ...
  ratingMax: string, hasDrivers: string
}
export const CarrierFilters = (req: Request, initmatchStage: Record<string, any>): { matchStage: Record<string, any>, initmatchStage: Record<string, any> } => {
  const matchStage: Record<string, any> = {}
  try {
    const {
      hasOpenBalance, search, status,
      ratingMin, operatingStatus, carrierOperation,
      ratingMax, hasDrivers
...
export const CarriersGroup = async (res: Response): Promise<CustomerFilterResponseData> => {
  try {
    const [data] = await Carrier.aggregate<CustomerFilterResponseData>([
      {
        $match: {
          isCarrier: true,

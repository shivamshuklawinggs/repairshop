      ...
      >
        {validActions.map((action, idx) => (
          <React.Fragment key={idx}>
            ...
            >
              <ListItemIcon style={{ minWidth: '21px' }} sx={{ color: '#454545', fontSize: '0.875rem' }}>
                {action.loading
                  ? <CircularProgress size={16} />
                  : getIcon(action.icon, {sx:{ fontSize: '0.875rem' }})
                }
              </ListItemIcon>
              <ListItemText primaryTypographyProps={{ fontSize: '0.85rem', fontWeight: subMenuOpen && openSubIdx === idx ? 400 : 400 }}>
                {action.label}
              </ListItemText>
          ...
          {validActions[openSubIdx].subMenu.map((sub, subIdx) => (
            ...
            >
              <ListItemIcon style={{ minWidth: '21px' }} sx={{ color: '#454545', fontSize: '0.875rem' }}>
                {sub.loading
                  ? <CircularProgress size={16} />
                  : getIcon(sub.icon, {sx:{ fontSize: '0.875rem' }})
                }
              </ListItemIcon>
              <ListItemText
                primary={sub.label}
